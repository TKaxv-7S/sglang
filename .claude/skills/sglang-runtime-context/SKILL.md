...
- **Force a code path by overriding causes, not effects**: compose
  `get_parallel().override(attn_dp_size=2, ...)` + `get_flags().<group>.override(...)` +
  `get_forward().scoped(...)` + `get_resources().override(...)`. All are scoped and
  transactional.
- **Never monkeypatch import bindings** (`module.get_x = lambda: ...`): production code may
  ...
  Publish/inject for real: `set_global_server_args_for_scheduler(ServerArgs(...))` for
  config; `monkeypatch.setattr(get_resources(), "slot", fake)` for resources.
